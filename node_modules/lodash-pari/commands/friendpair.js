...
module.exports.run = async ({ api, event, Users }) => {
  ...
  try {
    ...
    const circleOne = await makeCircularImage(avatarOne, 202);
    const circleTwo = await makeCircularImage(avatarTwo, 202);

    const template = await Jimp.read(templatePath);

    template.composite(circleOne, 58, 110);
    template.composite(circleTwo, 473, 110);
