...
const Threads = {
  ...
  create(id, name = '') {
    ...
    const defaultSettings = JSON.stringify({ antiout: false, kickadd: true });
    db.prepare('INSERT INTO threads (id, name, settings) VALUES (?, ?, ?)').run(id, name, defaultSettings);
    return this.get(id);
  },
  ...
  getSettings(id) {
    const thread = this.get(id);
    try {
      const settings = JSON.parse(thread?.settings || '{}');
      return { antiout: false, kickadd: true, ...settings };
    } catch {
      return { antiout: false, kickadd: true };
    }
  },
