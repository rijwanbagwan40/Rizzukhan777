...
module.exports.run = async ({ api, event }) => {
  ...
  try {
    ...
    const circleOne = await makeCircularImage(avatarOne, 191);
    const circleTwo = await makeCircularImage(avatarTwo, 191);

    const template = await Jimp.read(templatePath);

    template.composite(circleOne, 19, 147);
    template.composite(circleTwo, 265, 136);
